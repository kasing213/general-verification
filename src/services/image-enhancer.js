...
class ImageEnhancerService {
  constructor() {
    ...
    this.config = {
      autoEnhance: process.env.AUTO_ENHANCE_IMAGES !== 'false',
      deskew: process.env.ENABLE_DESKEW !== 'false',
  ...
  async enhanceImage(imageBuffer, options = {}) {
    ...
    if (!useOpenCV) {
      return this._enhanceImageFallback(imageBuffer, options, startTime);
    }
  ...
  /**
   * Main enhancement pipeline using Sharp and Jimp (fallback path)
   * @param {Buffer} imageBuffer - Original image buffer
   * @param {Object} options - Enhancement options
   * @param {number} startTime - Pipeline start timestamp
   * @returns {Promise<Object>} Enhancement result
   */
  async _enhanceImageFallback(imageBuffer, options = {}, startTime) {
    try {
  ...
  /**
   * Analyze image quality using Sharp metadata (fallback path)
  ...
  /**
   * Get enhancement recommendations based on quality score (fallback path)
  ...
  /**
   * Apply enhancements using Sharp (fallback path)
   * @param {Buffer} imageBuffer - Image buffer
  ...
  /**
   * Apply additional enhancements using Jimp for low-quality images (fallback path)
   * @param {Buffer} imageBuffer - Image buffer
   * @returns {Promise<Buffer>} Enhanced image buffer
   */
